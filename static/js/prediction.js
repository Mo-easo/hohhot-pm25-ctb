...
(function () {
  async function renderPlaceholder(domId) {
    ...
    const json = await window.Qingcheng.getJSON("/api/risk/forecast-placeholder");
    const hourly = json.data.hourly || [];
    const chart = echarts.init(el);
    chart.setOption({
      title: {
        text: "占位估计（非 ML）— " + (json.data.note || ""),
        left: 0,
        textStyle: { color: "#9bb5ad", fontSize: 12, width: 480, overflow: "break" },
      },
