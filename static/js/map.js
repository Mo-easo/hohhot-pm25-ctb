...
(function () {
  ...
  async function addStations(map, onClick) {
    const json = await window.Qingcheng.getJSON("/api/latest");
  ...
  async function initHomeMap(domId) {
    ...
    const center = (window.QINGCHENG && window.QINGCHENG.center) || { lat: 40.8421, lng: 111.749, zoom: 11 };
